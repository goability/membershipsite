...
if (!empty($this->ImageFilename)){
  $imagePath =  RESOURCE_DIR . '/' . $this->DB_Fields['ownerid'] . '/' . $this->Name . '/images/' . $this->ImageFilename;
}
?>
<div class="resourceForm">
  <div class="card">
    ...
    <div class="card-body">
      <h5 class="card-title"><?php echo $this->GetDisplayText();?></h5>
      <?php
        if (!empty($imagePath)){ ?>
    ...
    <div class="card-body">
          <table>
            <form class="" action="<?php echo($this->Classname . '?accessToken=' . PWH_SessionManager::GetAccessToken());?>" method="post">
              ...
              <input type="hidden" name="accessToken" value="<?php echo PWH_SessionManager::GetAccessToken();?>" id="accessToken">
                <?php
